...
def save_random_headers(random_headers_list, random_headers_file):
    with open(random_headers_file, 'w') as fp:
        ...
        print('Total number of selected sequences:', len(rand_headers))
        ...
        for index, seq_record in enumerate(SeqIO.parse(inp, "fasta")):
            ...
            if source_name not in source_names:
                ...
                if int(index) == last_index:
                    ...
                    idxs.append(idx)
        #print(source_names, idxs_list)

        rand_headers = []
        for i, name in zip(idxs_list, source_names):
            n = math.ceil(len(i) * fraction)
            print('for {} - {} seqs selected'.format(name, n))
            # select n randomly
            rand_header = [name + '_' + s for s in random.sample(i, n)]
